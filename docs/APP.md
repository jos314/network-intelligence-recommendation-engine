...
| Show top (10/25/50/100) | how many of the subject's top DIRECT counterparties (entities) or communities (clusters) form the baseline view |
| Rank by (Risk / Amount transacted / # Transactions) | which metric orders that top-N. **Risk** = calibrated score (default). **Amount transacted** / **# Transactions** = the node's total transacted volume / transaction count within the scored network (same numbers as the table and inspector) — surfaces the biggest money movers regardless of score. Applies to both views: in clusters it ranks and limits the communities, aggregating each community's member flows. The caption names the active metric ("top 25 of 480 direct counterparties by amount transacted") |
