...
from src.app.app import (CASES, DA, _conclusion_content, _counterparty_frame,
                         _elements, _decision_panel, _layout_spec,
                         _quickstats_panel, _render, _result, expand_top)
...
def test_render_all_cases(case_id):
    (els, style, layout, data, cols, decision, stats_panel, caption, drivers,
     paths, opts, theme, view_sig, table_note, filter_q, sort_by,
     search_val) = _render_case(case_id)
    nodes = [e for e in els if "source" not in e["data"]]
    edges = [e for e in els if "source" in e["data"]]
    assert nodes and edges and data and cols and decision and stats_panel
